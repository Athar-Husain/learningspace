...
<div class="h-custom-hero-area"
    ...
    <div class="h-custom-container">
        <div class="h-custom-column">
            <div class="h-banner-text-wrapper">
                <h6 class="h-come-for-learn-text h-landing-come-area">
                    @foreach (@$home->banner_mini_words_title ?? [] as $banner_mini_word)
                        <span>{{ __($banner_mini_word) }}</span>
                    @endforeach
                </h6>
                <h1 class="h-banner-title">
                    {{ __(@$home->banner_first_line_title) }}
                    <br>
                    {{ __(@$home->banner_third_line_title) }}
                </h1>
                <p class="h-banner-subtitle">{{ __(@$home->banner_subtitle) }}</p>
                ...
                <div class="h-button-group">
                    ...
                    <a href="{{ $home->banner_second_button_link }}" class="theme-btn theme-button1">
                        {{ __($home->banner_second_button_name) }}
                        <i class="fas fa-arrow-right mx-2"></i>
                    </a>
                </div>
            </div>
...
<div class="py-1 pb-28 mt-5 bg-white py-4">
    ...
    <div class="text-center mb-40">
        <h6 class="font-weight-semibold text-dark d-flex gap-3 justify-content-center">
            <img src="{{ asset('frontend/assets/img/icons/left-icon.png') }}" />{{ __('TRUSTED BY') }}
            <img src="{{ asset('frontend/assets/img/icons/right-icon.png') }}" />
        </h6>
        <h3 class="display-4 font-weight-bold text-dark my-10">{{ __(@$trustedSection->title ?? 'Top-Tier Entities') }}</h3>
        <div class="">
            <div class="row row-cols-5 row-cols-md-7 row-cols-lg-7 g-3 bg-white">
                @foreach (['trusted1', 'trusted2', 'trusted3', 'trusted4', 'trusted5', 'trusted6', 'trusted7', 'trusted8', 'trusted9', 'trusted10', 'trusted11', 'trusted12', 'trusted13', 'trusted14'] as $trusted)
                <div class="col px-2 py-1 d-flex justify-content-center align-items-center shadow-sm"
                    ...
                    <div class="d-flex justify-content-center align-items-center">
                        <img src="{{ getImageFile('frontend/assets/img/trusted/' . $trusted . '.png') }}" alt="Logo"
        ...
        <div class="d-flex justify-content-center mt-10">
            <a href="{{ $trustedSection->button_link ?? '/who-we-are' }}" class="theme-btn theme-button1 my-4">
                {{ __($trustedSection->button_name ?? 'View Success Stories') }}
                <i class="fas fa-arrow-right mx-2"></i>
            </a>
        </div>
    </div>
    ...
    <div class="container mt-5">
        <h3 class="text-3xl font-bold text-gray-900">{{ __(@$spaceSection->title ?? 'Why Our Space?') }}</h3>
        <div class="row row-cols-1 row-cols-md-3 g-4 mt-6">
                ...
                <div class="d-flex border-top border-black pt-3">
                    <div>
                        <h4 class="d-flex font-semibold text-xl align-items-center gap-3 my-4">
                            <img src="{{ getImageFile('frontend/assets/img/icons/space/1.png') }}" alt="icon" class="img-fluid" style="max-height: 15px;">
                            {{ __(@$spaceSection->feature_1_title ?? 'Market-Aligned Courses') }}
                        </h4>
                        <p class="text-gray-600">{{ __(@$spaceSection->feature_1_description ?? 'Learning Space helps in ramping your company’s performance and maximizing your productivity and profits.') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="col">
                <div class="d-flex border-top border-black pt-3">
                    <div>
                        <h4 class="d-flex font-semibold text-xl align-items-center gap-3 my-4">
                            <img src="{{ getImageFile('frontend/assets/img/icons/space/2.png') }}" alt="icon" class="img-fluid" style="max-height: 20px;">
                            {{ __(@$spaceSection->feature_2_title ?? 'Expert Instructors') }}
                        </h4>
                        <p class="text-gray-600">{{ __(@$spaceSection->feature_2_description ?? 'Learning Space helps in ramping your company’s performance and maximizing your productivity and profits.') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="col">
                <div class="d-flex border-top border-black pt-3">
                    <div>
                        <h4 class="d-flex font-semibold text-xl align-items-center gap-3 my-4">
                            <img src="{{ getImageFile('frontend/assets/img/icons/space/3.png') }}" alt="learning space" class="img-fluid" style="max-height: 20px;">
                            {{ __(@$spaceSection->feature_3_title ?? 'Flexible Materials') }}
                        </h4>
                        <p class="text-gray-600">{{ __(@$spaceSection->feature_3_description ?? 'Learning Space helps in ramping your company’s performance and maximizing your productivity and profits.') }}</p>
                    </div>
                </div>
            </div>
        </div>
...
<div class="h-skill h-skill-bg h-skill-padding">
    <div class="h-skill-container">
        <div class="h-skill-header">
            ...
            <div class="h-skill-text-left">
                <h2 class="h-skill-title text-white font-weight-bold">
                    {{ __(@$skillsSection->title ?? 'Elevate Your Skills.') }}<br />
                    {{ __(@$skillsSection->subtitle ?? 'Pick Your Subject') }}
                </h2>
                <p class="h-skill-description">
                    {{ __(@$skillsSection->description ?? 'We prepare individuals and organizations to excel in the tech-driven world, shaping the workforce of tomorrow.') }}
                </p>
            </div>
            <div class="h-skill-image-hidden">
                <img src="{{ getImageFile(@$skillsSection->image ?? 'frontend/assets/img/statisticscontainer.png') }}" alt="" class="img-fluid" style="max-width: 80%; height: auto;" />
            </div>
        </div>

        <!-- Course Categories -->
        <div class="h-skill-courses">
            @foreach (@$skillsSection->courses ?? [
                [
                    'icon' => 'software.png', 'title' => 'Software Development', 
                    'description' => 'Embedded System<br />UI Design<br />Software Engineering', 
                    'link' => '#'
                ],
                [
                    'icon' => 'bigdata.png', 'title' => 'Big Data & AI', 
                    'description' => 'AI<br />Business Intelligence<br />Software Engineering', 
                    'link' => '#'
                ],
                [
                    'icon' => 'cybersecurity.png', 'title' => 'Cyber Security', 
                    'description' => 'Information Security Engineering<br />Incident Response<br />Risk Analysis', 
                    'link' => '#'
                ],
                [
                    'icon' => 'infrastructure.png', 'title' => 'Infrastructure', 
                    'description' => 'Network Engineering<br />Cloud Computing', 
                    'link' => '#'
                ],
                [
                    'icon' => 'digitalmarketing.png', 'title' => 'E-commerce & Digital Marketing', 
                    'description' => 'Digital Marketing<br />Ecommerce', 
                    'link' => '#'
                ],
                [
                    'icon' => 'technical.png', 'title' => 'Technical Support & Operations', 
                    'description' => 'System Administration<br />Infrastructure Support', 
                    'link' => '#'
                ]
            ] as $course)
            <div class="h-skill-card">
                <div class="h-skill-card-icon">
                    <img src="{{ getImageFile('frontend/assets/img/icons/subject/' . $course['icon']) }}" alt="{{ __($course['title']) }}" class="h-skill-card-icon-img" />
                </div>
                <h3 class="h-skill-card-title">{{ __($course['title']) }}</h3>
                <p class="h-skill-card-description">
                    {!! $course['description'] !!}
                </p>
                <a href="{{ $course['link'] }}" class="h-skill-card-link">
                    {{ __('LEARN MORE') }}
                    <i class="fas fa-arrow-right mx-2"></i>
                </a>
            </div>
        ...
        <!-- View More Button -->
        <div class="d-flex justify-content-center mt-10">
            <a href="{{ @$skillsSection->view_more_link ?? '/courses' }}" class="theme-btn theme-button1 my-4">
                {{ __('View More') }}
                <i class="fas fa-arrow-right mx-2"></i>
            </a>
        </div>
    ...
    <div class="container mx-auto">
        ...
        <div class="home__grid grid grid-cols-1 md:grid-cols-2 gap-8 items-center bg-gray-100">
            ...
            <div class="home__image">
                <img src="{{ getImageFile(@$mainSection->image ?? 'frontend/assets/img/maximize.png') }}" alt="{{ __(@$mainSection->image_alt ?? 'Corporate Training') }}"
                    class="home__image--img w-full" />
            </div>
            <!-- Content -->
            <div class="home__content">
                <h2 class="home__title text-3xl md:text-4xl font-bold text-gray-900">
                    {{ __(@$mainSection->title ?? 'Maximize Company’s Performance') }}
                </h2>
                <p class="home__description mt-4 text-gray-600">
                    {{ __(@$mainSection->description ?? 'Unlock the full potential of your workforce with our carefully designed learning solutions. From leadership development to specialized technical skills, our programs are crafted to address your company\'s unique challenges.') }}
                </p>
                <a href="{{ @$mainSection->link ?? '/courses' }}" class="theme-btn theme-button1 my-4">
                    {{ __('View More') }}
                    <i class="fas fa-arrow-right mx-2"></i>
                </a>
            </div>
        ...
        <div class="home__partners text-center mt-16">
            <h3 class="home__partners-title text-xl font-semibold text-gray-900 flex gap-x-3 justify-center">
                <img src="{{ getImageFile('frontend/assets/img/icons/left-icon.png') }}" class="home__partners-icon" /> 
                {{ __('OUR PARTNERS') }}
                <img src="{{ getImageFile('frontend/assets/img/icons/right-icon.png') }}" class="home__partners-icon" />
            </h3>
            <div class="home__partners-logos mt-4 flex justify-center gap-16 flex-wrap items-center">
                @foreach (@$partners ?? [
                    ...
                    <img src="{{ getImageFile('frontend/assets/img/partners/' . $partner) }}" alt="{{ __('Partner') }}"
...
<div class="home__cta-section mt-16 py-16 text-center rounded-md bg-cover"
    ...
    <h2 class="home__cta-title text-3xl md:text-4xl font-bold text-gray-900">
        {{ __(@$ctaSection->cta_title ?? 'Don’t leave your progress to chance') }}
    </h2>
    <p class="home__cta-description mt-4 text-gray-600">
        {{ __(@$ctaSection->cta_description ?? 'Talk to one of our experts to show you how you can benefit from Learning Space.') }}
    </p>
    <div class="d-flex justify-content-center mt-10">
        <a href="{{ @$ctaSection->cta_link ?? '/contact-us' }}" class="theme-btn theme-button1 my-4">
            {{ __(@$ctaSection->cta_button_name ?? 'View More') }}
            <i class="fas fa-arrow-right mx-2"></i>
        </a>
    </div>
</div>
