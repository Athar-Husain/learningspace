...
class CourseController extends Controller
{
    ...
    // {
    //     $contactUsIssues = ContactUsIssue::all();
    //     return view('frontend.course.course', compact('contactUsIssues'));
    // }
    ...
    //     $home = Home::first();
    //     $contactUsIssues = ContactUsIssue::all();
    //     $featureCategories = Category::with('courses')->active()->get(); // Fetch categories with their courses
    //     $categoryTitle = get_option('category_course_title');
    //     $categorySubtitle = get_option('category_course_subtitle');
    //     $categoryLogo = get_option('category_course_logo');

    //     return view('frontend.course.course', compact('contactUsIssues', 'featureCategories', 'categoryTitle', 'categorySubtitle', 'categoryLogo'));
    // }

    public function coursesold()
    {
        // Fetch the first Home record
        $home = Home::first();

        // Fetch all contact issues
        $contactUsIssues = ContactUsIssue::all();

        // Fetch categories with their active courses
        $featureCategories = Category::with('courses')->active()->get();

        // Fetch category title, subtitle, and logo from settings (using the `get_option` helper)
        $categoryTitle = get_option('category_course_title');
        $categorySubtitle = get_option('category_course_subtitle');
        $categoryLogo = get_option('category_course_logo');

        // Pass all necessary data to the view
        return view('frontend.course.course', compact('home', 'contactUsIssues', 'featureCategories', 'categoryTitle', 'categorySubtitle', 'categoryLogo'));
    }


    public function coursesworking()
    {
        // Fetch the first Home record
        $home = Home::first();
        ...
        $featureCategories = Category::with('activeCourses')->with('courses.reviews')->with('courses.user')->with('courses.promotionCourse.promotion')->with('courses.instructor.ranking_level')->with('courses.specialPromotionTagCourse.specialPromotionTag')->active()->feature()->get()->map(function ($q) {
            $q->setRelation('courses', $q->courses->where('status', 1)->take(12));
            return $q;
        });

        // Fetch category title, subtitle, and logo from settings
        $categoryTitle = get_option('category_course_title');
        $categorySubtitle = get_option('category_course_subtitle');
        $categoryLogo = get_option('category_course_logo');

        // Get top courses (you can adjust this query as needed)
        $topCourse = Enrollment::query()
            ->whereMonth('created_at', now()->month)
            ->select('course_id', DB::raw('count(*) as total'))
            ->groupBy('course_id')
            ...
            ->orderBy('total', 'desc')
            ->get()
            ->pluck('course_id')  // Get an array of course IDs
            ->toArray();

        // Pass all necessary data to the view
        return view('frontend.course.course', compact('home', 'contactUsIssues', 'featureCategories', 'categoryTitle', 'categorySubtitle', 'categoryLogo', 'topCourse'));
    }


    public function coursesnew1()
    {
        // Fetch the first Home record
        $home = Home::first();
        ...
        // Fetch categories with their active courses
        $featureCategories = Category::with('activeCourses')
            ->with('courses.reviews')
            ->with('courses.user')
            ->with('courses.promotionCourse.promotion')
            ->with('courses.instructor.ranking_level')
            ->with('courses.specialPromotionTagCourse.specialPromotionTag')
            ->active()->feature()->get()
            ->map(function ($q) {
                $q->setRelation('courses', $q->courses->where('status', 1)->take(12));
                return $q;
        ...
        // Fetch top instructors based on the number of courses they are teaching or their popularity
        $topInstructors = User::whereHas('courses', function ($query) {
            $query->where('status', 1);
        })
            ->withCount('courses')
            ->orderBy('courses_count', 'desc')
            ->limit(6)  // Show top 6 instructors, adjust as needed
            ->get();

        // Pass all necessary data to the view
        return view('frontend.course.course', compact('home', 'contactUsIssues', 'featureCategories', 'categoryTitle', 'categorySubtitle', 'categoryLogo', 'topCourse', 'topInstructors'));
    }
    // public function courses()
    // {
    //     // Fetch the first Home record
    ...
    //         ->whereIn('users.role', [USER_ROLE_INSTRUCTOR]) // Filter only instructors
    //         ->where('ins.status', STATUS_APPROVED) // Only approved instructors
    //         ->with('badges')
    //         ->select('users.*', 'ins.organization_id')  // Remove selectStatement()
    //         ->withCount('enrollment_students')  // Include the count of enrollments for each instructor
    //         ->limit(6)  // Limit to top 6 instructors
    //         ->get();


    //     // Pass all necessary data to the view
    //     return view('frontend.course.course', compact('home', 'contactUsIssues', 'featureCategories', 'categoryTitle', 'categorySubtitle', 'categoryLogo', 'topCourse', 'instructors'));
    // }


    public function courses()
    {
        // Fetch course settings dynamically
        ...
        $instructors = User::query()
            ...
            ->with('badges')
            ->select('users.*', 'ins.organization_id')
            ->withCount('enrollment_students')
            ->limit(6)
            ->get();

        // Pass everything to the view
        return view('frontend.course.course', compact(
            'home',
            'contactUsIssues',
            'featureCategories',
            'categoryTitle',
