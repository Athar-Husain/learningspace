...
if (!function_exists('updateManifest')) {
    ...
    {
        $manifest = [
            "name" => get_option('app_name', 'LS'),
            "short_name" => get_option('app_name', 'LS'),
            "start_url" => route('main.index'),
            "background_color" => get_option('app_theme_color', '#5e3fd7'),
            "description" => get_option('app_name', 'LS'),
            "display" => "fullscreen",
...
if (!function_exists('isEnableOpenAI')) {
    ...
    {
        $role = auth()->user()?->role;
        return isAddonInstalled('LSAI') && in_array($role, [USER_ROLE_INSTRUCTOR, USER_ROLE_ORGANIZATION, USER_ROLE_ADMIN]) && get_option('open_ai_system');
    }
}
...
if (!function_exists('getThemePath')) {
    function getThemePath()
    {
        // Set default to '3' instead of THEME_DEFAULT
        $theme = get_option('theme', );

        // If the theme is '3', return 'frontend-theme-3'
        if ($theme == THEME_DEFAULT) {
            return 'frontend-theme-3';
        }
        return 'frontend-theme-' . $theme;
    }
}
